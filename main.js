  ...
  for (let j = 0; j < 7; j++) {
    ...
    if (el == "0") {
      ...
      if (el == "1") {
        ...
        fs.writeFile('date.txt', `2010-${date.month}-${date.day}`, (err) => {
          if (err) throw err;
          console.log('Data written to file successfully!');
          exec_cmd(`git add . ; git commit --date="2010-${date.month}-${date.day} 00:01:09" -m "today is 2010-${date.month}-${date.day}"; git push -u -f origin main`)
        });
        await sleep(3000);
      }
    }
